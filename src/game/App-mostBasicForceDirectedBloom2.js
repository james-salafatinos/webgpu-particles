...
function create() {
  ...
  const bloomPass = bloom(scenePassColor, 0.00, 0.1, 0.5);
  ...
  const attractionStrength = uniform(0.02); // Tweak this
  const repulsionStrength = uniform(0.01); // Tweak this
  const softening = float(0.05); // Avoid singularities at very small distances
  const dampening = uniform(0.9); // Adjust this to control the strength of dampening
